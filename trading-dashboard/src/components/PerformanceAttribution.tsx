      ...
      <div>
        ...
        <div className="card">
          <EmptyState
            ...
            subtitle="Performance attribution will appear after trades are executed. Run the paper trading bot or complete backtests to generate trade data."
          />
        </div>
      </div>
    )
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {Object.entries(data.by_direction).map(([dir, d]) => (
            <div key={dir} className="card">
              <div className="font-mono font-bold text-lg mb-2">{dir} Performance</div>
              <div className="text-sm text-txt-secondary">Trades: {d.total}</div>
              <div className="text-sm text-txt-secondary">Win Rate: {d.win_rate.toFixed(1)}%</div>
              <div className={d.total_pnl >= 0 ? 'text-accent-green font-mono mt-2' : 'text-accent-red font-mono mt-2'}>
