...
import { runBacktest, fetchBacktestResult } from '../../api'
...
export default function BacktestPanel() {
  ...
  const onRun = async () => {
    setBusy(true)
    try {
      const queued = await runBacktest({ strategy_name: strategy, initial_capital: capital })
      if (typeof queued?.id === 'string') {
        const details = await fetchBacktestResult(queued.id)
        setRun(details)
      }
    } finally {
      setBusy(false)
      ...
      <div className="card">
        {run ? (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div><div className="label">Sharpe</div><div>{run.sharpe_ratio ?? '-'}</div></div>
            <div><div className="label">Sortino</div><div>{run.sortino_ratio ?? '-'}</div></div>
