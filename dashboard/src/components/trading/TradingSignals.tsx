...
export default function TradingSignals() {
  ...
  const onGenerate = async () => {
    ...
    try {
      ...
      load()
      const generatedSignals = Array.isArray(result.signals) ? result.signals : []
      if (
        ...
        for (const signal of generatedSignals) {
          new Notification(`OutlierQ Signal: ${signal.ticker} ${signal.direction}`, {
            body: `${signal.strategy_name ?? signal.strategy} • ${Math.round((signal.confidence ?? 0) * 100)}% confidence`,
            icon: '/favicon.ico',
          })
        }
      }
